...
public class RecommendRecipe extends JPanel {
  ...
  JComboBox<String> combo1, combo2,
      combo3; //차례대로 사용처, 샐러디분류, 편의점분류 콤보박스
  ...
  public int menu1(int n) { // combo1(편의점, 샐러디 중 택1)에서 선택하는 값에
                            // 따라 패널의 가시성을 제어하는 메소드입니다.
    switch (n) {
    case 0:                 // 샐러디 선택시
  ...
  public void
  confirm_click(int n) { //확인 버튼 클릭시 콤보박스, 체크박스에 해당하는 값을
                         //가져와서 라벨의 내용을 바꾸는 메소드입니다.
    String m = "";
    ...
    case 0: // 샐러디 선택시
      ...
      if (b4.isSelected())
        m += b4.getText();
      if (b5.isSelected())
        m += b5.getText(); // b6은 "상관 없음"버튼 이라 값 가져올 필요 X

      m += "에 해당하는 리스트 나타날 자리";
      l.setText(
          m); //콤보박스, 체크박스에서 선택된 내용을 최종적으로 라벨에서 확인
      return;
  ...
  public void start() { //콤보박스, 체크박스, 버튼에서 발생하는 클릭 이벤트를
                        //처리하는 메소드입니다.
    combo1.addItemListener(
        e -> { menu1(combo1.getSelectedIndex()); }); //사용처 콤보박스
    combo2.addItemListener(
        e -> { b_Initialization(0); }); //(샐러디)분류 콤보박스

    confirm_b.addActionListener(
        e -> { confirm_click(menu1(combo1.getSelectedIndex())); }); //확인 버튼

    b1.addActionListener(e -> {
      b6_Initialization();
    }); //체크박스 1/2/3/4/5에 이벤트 발생시 "상관없음" 버튼(6)을 초기화합니다.
    ...
    b6.addActionListener(e -> {
      b_Initialization(1);
    }); //체크박스에서 "상관없음"(6)을 선택시 나머지 체크박스(1/2/3/4/5)가
        //초기화됩니다.
  }
  ...
  public static void main(String[] args) {
    ...
    frame.setBounds(300, 200, 600, 450); //앞: 프로그램 실행시 화면 내 x/y 좌표
                                         //+ 뒤: 프로그램 가로 세로 길이
