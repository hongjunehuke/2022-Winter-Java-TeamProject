...
class Recipe {
  ...
  public static ArrayList<String> filtered_recipes_name =
      new ArrayList<String>();

  Recipe(String id, String name, String cate, String cate2, String ingredient,
         String recipe, String image) {
    this.id = id;
    this.name = name;
    this.cate = cate;
    this.cate2 = cate2;
  ...
  public static void filter(String cate, String cate2) {
    ...
    for (Recipe aRecipe : Recipe.recipes)
      if (aRecipe.object_filter(cate, cate2))
        filtered_recipes.add(aRecipe); // Recipe 객체의 object_filter() 결과가
                                       // t일때만 filtered_recipes에 추가
  }

  public boolean object_filter(String cate, String cate2) {
    if (this.cate.equals(cate))
      if (cate2.equals("x"))
        return true; // 샐러디 체크박스 "상관없음" 선택시 cate2가 x로 들어옴. 이
                     // 경우 cate1만 일치해도 다 t 반환
      else if (cate2.contains(this.cate2))
        return true; // 이외 버튼 클릭시 포함관계 따져서 this.cate2가 cate2에
                     // 들어있으면 t 반환
      else
        return false;
...
public class RecommendRecipe extends JPanel {
  ...
  RecommendRecipe() {
    ...
    list.setSelectionMode(
        ListSelectionModel
            .SINGLE_SELECTION); // 리스트에서 하나의 요소만 선택 가능하게 설정
  ...
  public int
  menu(int n) { // combo1에서 선택하는 값에 따라 가시성을 제어하는 메소드입니다.
    switch (n) {
    case 0:                    // 샐러디
  ...
  public void
  clickConfirm(int n) { // 확인버튼 클릭시 콤보박스, 체크박스에 해당하는 값을
                        // 가져와서 Recipe 클래스의 필터링 메소드 호출
    String m = "";
    ...
    case 0: // combo1에서 샐러디 선택시
      ...
      Recipe.filter((String)combo2.getSelectedItem(),
                    m); // 샐러디는 체크박스 내용을 문자열 하나로 묶어서 보냄
      return;

    case 1: // combo1에서 편의점 선택시
      Recipe.filter((String)combo3.getSelectedItem(),
                    "x"); // 편의점은 분류가 1개라서 2번째 카테고리는 x로 넣음
      return;
  ...
  public void
  b_Initialization(int n) { // 체크박스의 상태를 초기화하는 메소드입니다.
    ...
    if (n == 0)
      b6.setSelected(false); // 콤보박스에 이벤트 발생할때만 파라미터를 0으로
                             // 줘서 마지막 버튼까지 초기화
  }

  public void b6_Initialization() { // "상관없음" 선택 후 다시 다른 체크박스를
                                    // 클릭하면 "상관없음" 버튼에서 체크가
                                    // 해제되도록 하는 역할의 메소드입니다.
    if (b1.isSelected() || b2.isSelected() || b3.isSelected() ||
        b4.isSelected() || b5.isSelected())
      b6.setSelected(false);
  }

  public void start() { // 클릭 이벤트를 처리하는 메소드입니다.
    combo1.addItemListener(
        e -> { menu(combo1.getSelectedIndex()); }); // 사용처 콤보박스
    combo2.addItemListener(e -> {
      b_Initialization(0);
    }); // 샐러디 분류 콤보박스에 이벤트 발생시 체크박스 모두 초기화
    ...
    b1.addActionListener(e -> {
      b6_Initialization();
    }); // 체크박스 1/2/3/4/5에 이벤트 발생시 "상관없음" 버튼(6) 초기화
    ...
    b6.addActionListener(e -> {
      b_Initialization(1);
    }); // 체크박스에서 "상관없음"(6) 선택시 나머지 체크박스(1/2/3/4/5)가 초기화
  ...
  public static void main(String[] args) {
    try {
      // Path path =
      // Paths.get("C:\\Users\\Jeongyeon\\Desktop\\temp_recipe.txt"); Path path
      // = Paths.get("경로");
      ...
      for (int j = 7; j < Arr.length;) // 7부터 시작하는 이유 : 처음에 id(0) ~
                                       // image(6) 건너뛰려고
        new Recipe(Arr[j++], Arr[j++], Arr[j++], Arr[j++], Arr[j++], Arr[j++],
                   Arr[j++]); // 레시피 하나 가져와서 Recipe 객체 만들기
    ...
    frame.setBounds(300, 200, 600, 400); // 앞: 프로그램 실행시 화면 내 x/y 좌표
                                         // + 뒤: 프로그램 가로 세로 길이
